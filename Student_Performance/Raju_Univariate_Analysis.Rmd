...
# Combined frequency table for all binary variables
binary_summary <- data.frame(
  Variable = c("sex (Female)", "sex (Male)", "paid (yes)", "paid (no)",
               "activities (yes)", "activities (no)", "higher (yes)", "higher (no)",
               "internet (yes)", "internet (no)"),
  Absolute_Freq = c(sum(data$sex == "F"), sum(data$sex == "M"),
                    sum(data$paid == "yes"), sum(data$paid == "no"),
                    sum(data$activities == "yes"), sum(data$activities == "no"),
                    sum(data$higher == "yes"), sum(data$higher == "no"),
                    sum(data$internet == "yes"), sum(data$internet == "no")),
  Relative_Freq = c(round(mean(data$sex == "F"), 3), round(mean(data$sex == "M"), 3),
                    round(mean(data$paid == "yes"), 3), round(mean(data$paid == "no"), 3),
                    round(mean(data$activities == "yes"), 3), round(mean(data$activities == "no"), 3),
                    round(mean(data$higher == "yes"), 3), round(mean(data$higher == "no"), 3),
                    round(mean(data$internet == "yes"), 3), round(mean(data$internet == "no"), 3))
)
kable(binary_summary, caption = "Frequency Distribution of Binary Variables")
```

```{r binary-combined-plot, fig.width=10, fig.height=5}
# Create data for combined plot (showing "yes" or positive category percentages)
binary_data <- data.frame(
  Variable = factor(c("Sex (Female)", "Paid Classes", "Activities", "Higher Ed", "Internet"),
                    levels = c("Sex (Female)", "Paid Classes", "Activities", "Higher Ed", "Internet")),
  Percentage = c(
    mean(data$sex == "F") * 100,
    mean(data$paid == "yes") * 100,
    mean(data$activities == "yes") * 100,
    mean(data$higher == "yes") * 100,
...
ggplot(binary_data, aes(x = Variable, y = Percentage, fill = Variable)) +
  ...
  labs(title = "Distribution of Binary Variables (Percentage of Yes/Positive Category)",
       x = "",
       y = "Percentage (%)") +
  theme_minimal() +
...
- **Sex:** The dataset contains `r sum(data$sex == "F")` female (`r round(mean(data$sex == "F") * 100, 1)`%) and `r sum(data$sex == "M")` male (`r round(mean(data$sex == "M") * 100, 1)`%) students - relatively balanced.
- **Paid classes:** `r round(mean(data$paid == "yes") * 100, 1)`% of students take extra paid math classes.
- **Activities:** `r round(mean(data$activities == "yes") * 100, 1)`% participate in extra-curricular activities.
- **Higher education:** An overwhelming `r round(mean(data$higher == "yes") * 100, 1)`% want to pursue higher education.
- **Internet:** `r round(mean(data$internet == "yes") * 100, 1)`% have internet access at home.
