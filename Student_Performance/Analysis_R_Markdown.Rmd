...
```{r raju-binary-combined, fig.width=9, fig.height=2.5, fig.pos='H', fig.cap="Binary Variables - Summary Table (left) and Grouped Bar Chart (right)"}
binary_summary <- data.frame(
  Variable = c("Paid Extra Classes", "Higher Education Aspiration", "Internet Access"),
  Count_Yes = c(sum(df$paid == "yes"), sum(df$higher == "yes"), sum(df$internet == "yes")),
  Count_No = c(sum(df$paid == "no"), sum(df$higher == "no"), sum(df$internet == "no")),
  Mode = c("no", "yes", "yes")
)

# Create table grob
table_grob <- tableGrob(binary_summary, rows = NULL,
                        cols = c("Variable", "Yes", "No", "Mode"),
                        theme = ttheme_default(
...
grid.arrange(table_grob, plot_grob, ncol = 2, widths = c(0.45, 0.55))
```
...
```{r hrush-studytime-combined, fig.width=7, fig.height=2.5, fig.pos='H', fig.cap="Study Time vs G3: Statistics (left) and distribution (right)"}
...
grid.arrange(p_table, p_plot, ncol = 2, widths = c(1, 1.8))
```

**Interpretation:** Spearman r=`r round(spearman_studytime, 3)` indicates moderate positive relationship. Students studying 5-10 hours achieve highest mean (12.61), while <2 hours group has lowest (9.13). Most students (n=197) study 2-5 hours weekly. Study time is a meaningful predictor, though extreme hours may reflect diminishing returns.
...
```{r hrush-failures-combined, fig.width=7, fig.height=2.5, fig.pos='H', fig.cap="Failures vs Absences: Statistics (left) and distribution (right)"}
...
p_plot <- ggplot(df, aes(x = failures_factor, y = absences, fill = failures_factor)) +
  ...
  labs(title = "Absences vs. Past Failures",
       subtitle = paste("Spearman r =", round(spearman_failures, 3)),
...
**Interpretation:** Spearman r=`r round(spearman_failures, 3)` reveals weak positive relationship. Students with no failures average 5.23 absences, while those with 3 failures average 8.18. High variability and outliers suggest the relationship is not deterministic. Most failure groups cluster near typical absence levels, indicating factors beyond attendance contribute to academic failure.
...
```{r hrush-paid-combined, fig.width=7, fig.height=2.5, fig.pos='H', fig.cap="Paid Classes vs G3: Statistics (left) and density distribution (right)"}
...
**Interpretation:** Students without paid classes have higher mean grade (10.75) vs. those with paid classes (9.90), difference of 0.85 points. T-test p=`r round(t_test_result$p.value, 4)` suggests marginally significant difference. Counterintuitive finding likely reflects selection bias: struggling students more likely enroll for remedial support. Substantial distribution overlap indicates paid classes do not universally improve performance.
